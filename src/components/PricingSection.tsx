...
export default function PricingSection() {
  return (
    <section
      id="pricing"
      className="relative w-full px-6 md:px-12 py-24 sm:py-32 lg:py-36 overflow-hidden"
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center">
        ...
        <h2 className="hero-headline mt-8 text-center text-4xl sm:text-5xl md:text-6xl text-gradient">
        ...
        <div className="mt-16 grid w-full grid-cols-1 gap-8 md:grid-cols-3">
          {tiers.map((tier) => (
            <div key={tier.name} className="group relative">
              <div className="absolute inset-0 rounded-[28px] bg-gradient-to-br from-white/20 via-white/5 to-transparent opacity-0 blur-3xl transition duration-500 group-hover:opacity-100" />
              <div
                className={`relative flex h-full flex-col rounded-[28px] border border-white/10 bg-white/5 p-9 backdrop-blur-2xl transition-transform duration-500 group-hover:-translate-y-2 ${
                  tier.popular
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {tier.popular && (
                    <span className="inline-flex items-center rounded-full bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-wide text-white shadow-lg shadow-purple-700/30">
                      Most Popular
                    </span>
                  )}
                </div>
                ...
                <p className="mt-2 text-sm text-white/50">{tier.priceCaption}</p>

                <ul className="mt-10 flex flex-1 flex-col gap-4 text-left">
